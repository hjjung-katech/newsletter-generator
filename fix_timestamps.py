...
def is_kst_timestamp(timestamp_str):
    ...
    try:
        ...
        if 'T' in timestamp_str and not timestamp_str.endswith('Z') and '+' not in timestamp_str and '-' not in timestamp_str[-6:]:
...
def fix_database_timestamps(db_path="storage.db"):
    ...
    try:
        ...
        cursor.execute("SELECT id, next_run, created_at FROM schedules WHERE next_run LIKE '%T%' OR created_at LIKE '%T%'")
...
def verify_timestamps(db_path="storage.db"):
    ...
    # History 테이블 확인
    cursor.execute("SELECT id, created_at FROM history ORDER BY created_at DESC LIMIT 5")
