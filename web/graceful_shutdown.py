...
class GracefulShutdownWrapper:
    ...
    def _register_flask_shutdown_tasks(self):
        ...
        # (handled by ShutdownManager's wait phase)
        register_shutdown_task(
    ...
    def _setup_flask_hooks(self):
        ...
        def teardown_request(exception):
            ...
            if request_id:
                ...
                if exception:
                    logger.debug(
                        f"Request {request_id} failed with exception: {exception}"
                    )
    ...
    def _wait_for_active_requests(self):
        ...
        if remaining_count > 0:
            logger.warning(
                "Timeout waiting for requests - "
                f"{remaining_count} requests still active"
            )
    ...
    def run(
        self, host: str = "0.0.0.0", port: int = 8000, debug: bool = False, **options
    ):
        """
        ...
        if debug:
            logger.warning(
                "Debug mode enabled - graceful shutdown may not work properly"
            )
    ...
    try:
        ...
        def patched_make_server(*args, **kwargs):
            ...
            def shutdown_handler(signum, frame):
                logger.info(
                    f"Server received signal {signum} - starting graceful shutdown"
                )
