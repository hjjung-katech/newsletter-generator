...
def compose_newsletter_html(data: dict, template_dir: str, template_name: str) -> str:
    """
    Generates HTML newsletter content from structured data using a Jinja2 template.

    Args:
        data (dict): A dictionary containing all necessary data for the newsletter.
    ...
    """
    print(f"Composing newsletter for topic: {data.get('newsletter_topic', 'N/A')}...")
    env = Environment(
    ...
    # Ensure generation_date is available, defaulting to today if not provided
    generation_date = data.get('generation_date', datetime.now().strftime("%Y-%m-%d"))
    ...
    context = {
        ...
        'introduction_message': data.get('introduction_message', '지난 한 주간의 주요 산업 동향을 정리해 드립니다.'),
        'sections': data.get('sections', []),
        'food_for_thought': data.get('food_for_thought'), # Can be None
        'closing_message': data.get('closing_message', '다음 주에 더 유익한 정보로 찾아뵙겠습니다. 감사합니다.'),
...
if __name__ == '__main__':
        ...
        'sections': [
            {
                ...
                'definitions': [
                    {'term': "AI 신약 개발", 'explanation': "인공지능 기술을 활용하여 신약 후보물질 발굴, 약물 설계, 임상시험 최적화 등의 과정을 개선하는 연구 분야입니다."}
                ],
                'news_links': [
                    {'title': "[PDF] AI를 활용한 혁신 신약개발의 동향 및 정책 시사점", 'url': "https://www.kistep.re.kr/boardDownload.es?bid=0031&list_no=94091&seq=1", 'source_and_date': "KISTEP"},
                    {'title': "제약바이오, AI 신약개발 박차…\"패러다임 바뀐다\"", 'url': "https://www.kpanews.co.kr/article/show.asp?idx=256331&category=D", 'source_and_date': "KPA News"},
                ]
            },
            {
                'title': "디지털 치료제",
                ...
                'definitions': [
                    {'term': "디지털 치료제", 'explanation': "소프트웨어 형태의 의료기기로, 질병의 예방, 관리, 치료를 목적으로 합니다. 주로 앱, 게임, 웨어러블 기기 등을 통해 제공됩니다."}
                ],
                'news_links': [
                    {'title': "디지털 치료제 - 나무위키", 'url': "https://namu.wiki/w/%EB%94%94%EC%A7%80%ED%84%B8%20%EC%B9%98%EB%A3%8C%EC%A0%9C", 'source_and_date': "Namuwiki"}
                ]
            }
            # ... more sections ...
        ],
    ...
    project_root = os.path.dirname(current_dir) # Moves up one level to the project root
    ...
    if not os.path.isdir(template_directory):
        ...
        print(f"Error: Template file not found at {os.path.join(template_directory, template_file)}")
    else:
        print(f"Template directory: {template_directory}")
        print(f"Template file: {template_file}")
        try:
            html_output = compose_newsletter_html(example_data, template_directory, template_file)
            output_filename = os.path.join(project_root, 'output', f"composed_newsletter_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html")
