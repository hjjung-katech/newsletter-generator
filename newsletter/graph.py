...
def parse_article_date_for_graph(date_str: Any) -> Optional[datetime]: # Changed type hint for initial check
...
# 노드 함수 정의
def collect_articles_node(state: NewsletterState) -> NewsletterState: # Renamed for clarity
    """
...
def process_articles_node(state: NewsletterState) -> NewsletterState:
    ...
    if not collected_articles or not isinstance(collected_articles, list):
        print("[yellow]Warning: No articles collected or 'collected_articles' is not a list. Skipping processing.[/yellow]")
        return {
            **state,
            "processed_articles": [],
    ...
    if not recent_articles:
        ...
        processed_output_filename = f"{timestamp}_collected_articles_processed_EMPTY.json"
    ...
    for article in recent_articles:
        ...
        if url and isinstance(url, str) and url.strip():
            normalized_url = url.lower().strip().rstrip('/') # Normalize by lowercasing and removing trailing slash
            if normalized_url not in unique_articles_dict:
                unique_articles_dict[normalized_url] = article
            else:
                print(f"Debug: Deduplicated article (URL already seen): {article.get('title', 'N/A')}, URL: {url}")
        else:
            print(f"Warning: Article missing URL or URL is not a string, cannot deduplicate: {article.get('title', 'N/A')}")
    ...
    if not deduplicated_articles:
        ...
        # Optionally, save an empty processed file or skip
        processed_output_filename = f"{timestamp}_collected_articles_processed_EMPTY_DEDUP.json"
        processed_output_path = os.path.join(output_dir, processed_output_filename)
        try:
            with open(processed_output_path, 'w', encoding='utf-8') as f:
                json.dump([], f, ensure_ascii=False, indent=4)
            print(f"Saved empty processed articles list (post-dedup) to {processed_output_path}")
        except Exception as e:
            print(f"[red]Error saving empty processed articles list: {e}[/red]")
...
def summarize_articles_node(state: NewsletterState) -> NewsletterState: # Renamed for clarity
    """
    수집된 기사를 요약하는 노드
    """
...
def create_newsletter_graph() -> StateGraph:
    ...
    workflow.add_conditional_edges(
        "collect_articles",
        lambda state: "handle_error" if state.get("status") == "error" else "process_articles" # Route to process_articles
    )
    workflow.add_conditional_edges(
        "process_articles", # Edges from new node
        lambda state: "handle_error" if state.get("status") == "error" else ("summarize_articles" if state.get("processed_articles") else "handle_error")
    )
    workflow.add_conditional_edges(
        "summarize_articles",
...
def generate_newsletter(keywords: List[str]) -> Tuple[str, str]:
    ...
    # 결과 반환
    if final_state.get("status") == "complete" and final_state.get("newsletter_html") is not None:
