        ...
        class SafeLogger:
            ...
            def _safe_log(self, level, msg, *args, **kwargs):
                try:
                    ...
                    # Stream closed during test teardown or other logging errors.
                    pass
    ...
    def register_shutdown_task(
        ...
        **kwargs,
        ...
        with self._lock:
            ...
            self._safe_log.debug(
                "Registered shutdown task: "
                f"{name} (phase: {phase.value}, priority: {priority})"
            )
            return True
    ...
    def _wait_for_background_tasks(self, timeout: float) -> bool:
        ...
        while time.time() - start_time < timeout:
            with self._lock:
                ...
                self._safe_log.debug(
                    "Waiting for "
                    f"{len(active_threads)} threads and "
                    f"{len(active_processes)} processes"
                )
        ...
        with self._lock:
            if self._running_threads or self._running_processes:
                self._safe_log.warning(
                    ...
                    f"{len(self._running_threads)} threads, "
                    f"{len(self._running_processes)} processes still running"
                )
    ...
    def _force_shutdown(self) -> bool:
        ...
        with self._lock:
            if self._running_threads:
                self._safe_log.warning(
                    "Cannot force terminate "
                    f"{len(self._running_threads)} threads - they will be abandoned"
                )
                for name in self._running_threads:
