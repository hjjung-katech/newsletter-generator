...
class ShutdownManager:
        ...
        def signal_handler(signum, frame):
            ...
            with self._lock:
                ...
                if self._shutdown_request_count >= 2:
                    ...
                    if elapsed < 3.0:
                        self._safe_log.warning(
                            f"Second shutdown signal received within {elapsed:.1f}s - forcing exit"
                        )
                        import os
                ...
                if self._shutdown_completed:
                    self._safe_log.warning(
                        "Shutdown already completed but still receiving signals - forcing exit"
                    )
                    import os
            ...
            def console_ctrl_handler(ctrl_type):
                ...
                with self._lock:
                    ...
                    if self._shutdown_request_count >= 2:
                        ...
                        if elapsed < 3.0:
                            self._safe_log.warning(
                                f"Second console event received within {elapsed:.1f}s - forcing exit"
                            )
                            import os
                    ...
                    if self._shutdown_completed:
                        self._safe_log.warning(
                            "Shutdown already completed but still receiving console events - forcing exit"
                        )
                        import os
        ...
        **kwargs,
        ...
        with self._lock:
            if self._shutdown_requested:
                self._safe_log.warning(
                    f"Cannot register task '{name}' - shutdown already requested"
                )
                return False
            ...
            self._safe_log.debug(
                f"Registered shutdown task: {name} (phase: {phase.value}, priority: {priority})"
            )
            return True
    ...
    def register_thread(self, name: str, thread: threading.Thread) -> bool:
        ...
        with self._lock:
            if self._shutdown_requested:
                self._safe_log.warning(
                    f"Cannot register thread '{name}' - shutdown already requested"
                )
                return False
    ...
    def register_process(self, name: str, process: Any) -> bool:
        ...
        with self._lock:
            if self._shutdown_requested:
                self._safe_log.warning(
                    f"Cannot register process '{name}' - shutdown already requested"
                )
                return False
    ...
    def shutdown(self, force: bool = False) -> bool:
        ...
        if force:
            self._safe_log.warning(
                "Force shutdown requested - skipping graceful phases"
            )
    ...
    def _execute_phase(self, phase: ShutdownPhase) -> bool:
        ...
        self._safe_log.info(
            f"--- Starting phase: {phase.value} (timeout: {phase_timeout}s) ---"
        )
        ...
        phase_tasks = [
            ...
            if task.phase == phase and not task.completed
        ]
        ...
        for task in phase_tasks:
            ...
            if remaining_time <= 0:
                self._safe_log.warning(
                    f"Phase {phase.value} timeout reached before task '{task.name}'"
                )
                break
        ...
        self._safe_log.info(
            f"--- Completed phase: {phase.value} in {phase_duration:.2f}s ---"
        )
        ...
        def run_task():
            try:
                ...
                self._safe_log.error(
                    f"Error in shutdown task '{task.name}': {e}", exc_info=True
                )
    ...
    def _wait_for_background_tasks(self, timeout: float) -> bool:
        ...
        while time.time() - start_time < timeout:
            with self._lock:
                ...
                self._safe_log.debug(
                    f"Waiting for {len(active_threads)} threads and {len(active_processes)} processes"
                )
    ...
    def _force_shutdown(self) -> bool:
        ...
        with self._lock:
            if self._running_threads:
                self._safe_log.warning(
                    f"Cannot force terminate {len(self._running_threads)} threads - they will be abandoned"
                )
                for name in self._running_threads:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "completed_tasks": len(
                    [t for t in self._shutdown_tasks if t.completed]
                ),
