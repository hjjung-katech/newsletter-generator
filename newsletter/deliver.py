...
def process_html_for_email(html_content: str) -> str:
    ...
    if PREMAILER_AVAILABLE:
        try:
            ...
            print(f"Warning: Failed to process HTML with premailer: {e}")
            return cleaned_html
    else:
        return cleaned_html
...
def send_email(to_email: str, subject: str, html_content: str):
    ...
    print(f"   제목: {subject}")
    print(f"   내용 길이: {len(html_content)} 문자")

    # EMAIL_SENDER 설정 확인
    if not config.EMAIL_SENDER:
        print("❌ 오류: EMAIL_SENDER가 설정되지 않았습니다!")
        print("💡 해결 방법:")
        print("   .env 파일에 다음을 추가하세요:")
        print("   EMAIL_SENDER=your_verified_sender@example.com")
        print("   (주의: Postmark에서 인증된 이메일 주소여야 합니다)")
        return False

    print(f"   발송자: {config.EMAIL_SENDER}")

    # POSTMARK_SERVER_TOKEN 설정 확인
    if not config.POSTMARK_SERVER_TOKEN:
        print("❌ 오류: POSTMARK_SERVER_TOKEN이 설정되지 않았습니다!")
        print("💡 해결 방법:")
        print("   .env 파일에 다음을 추가하세요:")
        print("   POSTMARK_SERVER_TOKEN=your_postmark_server_token")
        print("   (Postmark 대시보드 → Server → API Tokens에서 확인 가능)")
        print("⚠️  CI 환경에서는 경고로 처리되어 성공으로 간주됩니다.")
    ...
    print(f"   Postmark 토큰: {masked_token}")

    cleaned_html_content = process_html_for_email(html_content)

    try:
        print(f"📤 Postmark API 호출 중...")
            ...
            if response.status_code == 422:
                try:
                    ...
                    if error_code == 406:
                        ...
                        print("   2. Postmark 대시보드 → Signatures에서 인증 상태 확인")
                        print(f"   3. 현재 설정: {config.EMAIL_SENDER}")
                    else:
                        print("\n💡 일반적인 해결 방법:")
                        print("   1. 이메일 주소 형식 확인")
                        print("   2. Postmark 대시보드에서 발송자 서명 인증 확인")
                        print("   3. 월별 발송 한도 초과 여부 확인")
                except Exception as parse_error:
                    print(f"   (오류 응답 파싱 실패: {parse_error})")

            elif response.status_code == 401:
                print("\n💡 해결 방법 (인증 실패):")
                print("   1. POSTMARK_SERVER_TOKEN이 올바른지 확인하세요")
                print("   2. Postmark 대시보드에서 토큰을 다시 확인하세요")
                print("   3. 토큰이 Server Token인지 확인 (Account Token 아님)")
                print(f"   4. 현재 토큰: {masked_token}")

            elif response.status_code == 403:
                print("\n💡 해결 방법 (권한 부족):")
                print("   1. Postmark 계정의 발송 권한 확인")
                print("   2. 계정이 일시 중지되었는지 확인")
                print("   3. 발송 한도 초과 여부 확인")

            return False

    except requests.exceptions.Timeout:
        print("❌ 이메일 발송 실패: 요청 시간 초과")
        print("💡 해결 방법:")
        ...
        print("❌ 이메일 발송 실패: 연결 오류")
        print("💡 해결 방법:")
        print("   1. 인터넷 연결 확인")
        print("   2. 방화벽 설정 확인")
        print("   3. VPN 사용 중인 경우 해제 후 재시도")
        return False
        ...
        print("💡 해결 방법:")
        print("   1. 네트워크 연결 확인")
        print("   2. 설정 파일(.env) 확인")
        print("   3. 문제가 지속되면 개발팀에 문의")
        return False
