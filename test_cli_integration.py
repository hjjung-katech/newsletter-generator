...
class RealNewsletterCLI:
    ...
    def generate_newsletter(self, keywords=None, domain=None, template_style="compact", email_compatible=False, period=14):
        """실제 CLI를 사용하여 뉴스레터 생성"""
        try:
            # CLI 명령어 구성
            cmd = [
                sys.executable, '-m', 'newsletter.cli', 'run',
                '--output-format', 'html',
            ...
            if html_content:
                # 제목 추출
                title = self._extract_title_from_html(html_content) or f"Newsletter: {keywords or domain}"
