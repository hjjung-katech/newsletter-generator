...
## 5. 릴리즈 surface와 메타데이터/무결성 생성

Windows canonical desktop release surface는 아래 4개를 기준으로 유지합니다.
...
`dist\update-manifest.json`은 업데이트 채널을 실제로 운영하는 release-gated 경로에서만 추가됩니다. CI도 이 조건부 surface를 그대로 검증합니다.
...
# 업데이트 채널을 운영하는 release-gated 경로에서만 생성
...
python scripts/devtools/validate_windows_release_artifacts.py --dist-dir dist --require-update-manifest
python scripts/devtools/check_legacy_web_types_paths.py
```

- `dist\release-metadata.json`: 버전/빌드시각/Git SHA/Smoke 결과 등 릴리즈 메타데이터
- `dist\SHA256SUMS.txt`: 기본적으로 `newsletter_web.exe`, `release-metadata.json`, `support-bundle.zip` 무결성 체크섬을 포함하고, `update-manifest.json`을 생성한 경우 해당 파일도 포함합니다
- `dist\support-bundle.zip`: 지원/장애 재현용 마스킹 진단 번들(`bundle-manifest.json`, `recent-errors.txt` 포함)
- `dist\update-manifest.json`: 선택적 업데이트 채널용 추가 메타데이터(조건부 release surface)
