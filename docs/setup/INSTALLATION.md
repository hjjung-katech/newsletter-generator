...
- **Python**: 3.11 또는 3.12
- **운영체제 지원 계약**:
  - Linux: source 개발 + canonical production server
  - Windows: source 개발 + native bundle 정식 지원
  - macOS: source 개발 및 smoke 중심
...
추가 정책:
- Python 3.10은 legacy transition only 상태이며 2026-06-30까지 migration-only로 간주합니다.
- Python 3.13은 experimental source-only 상태입니다.
- 정확한 OS minor version floor는 아직 저장소 계약으로 고정되지 않았습니다.
...
`make bootstrap`, `make doctor`, `make check`는 같은 Python 엔트리포인트를 호출하는 thin wrapper로 유지됩니다.
...
```bash
# 오류: Python 3.11 또는 3.12 필요
# 해결: Python 업그레이드
python --version
# Python 3.10.x 이하인 경우 3.11/3.12 환경 생성 권장
```
