...
- settings provenance 운영 가시성은 위 개별 visibility를 다시 조합하는 `web/settings_provenance_support.py` 와 route helper layer가 additive `effective_settings_provenance` payload를 만들고, 같은 helper가 detached / mismatched / empty / default-only 상태에 대한 additive diagnostics reason layer도 함께 계산합니다. 현재 diagnostics payload는 reason code/summary 뿐 아니라 field-level explanation(`field_summary`, `field_explanations`)도 포함해 어떤 축과 어떤 필드가 기대값/현재값에서 어긋났는지까지 additive 하게 설명합니다. `web/static/js/app_view_state_helpers.js` 와 `web/static/js/app_request_response_helpers.js` 는 preset / source policy / personalization / recent execution 을 하나의 provenance summary와 diagnostics summary로 정규화해 사용자가 현재 effective settings 조합과 그 근거를 같은 방식으로 해석하도록 유지합니다.
