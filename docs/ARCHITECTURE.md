...
- 2026-03-14 기준 위 다섯 hotspot(`newsletter/llm_factory.py`, `newsletter/tools.py`, `newsletter/graph.py`, `web/routes_generation.py`, `web/static/js/app.js`)은 maintenance mode 대상으로 간주합니다. 다음부터는 user-facing bug fix, 새로운 제품 요구, 중복 로직 재유입, shell complexity 재상승 같은 reopen 조건이 있을 때만 다시 구조 변경 대상으로 다룹니다.
