...
- `web/routes_generation.py` 는 여전히 Flask wiring, request/app context, DB/task side-effect 경계를 담당하는 legacy route shell이지만, request parsing/validation, preview/schedule option normalization, sync response shaping은 `web/generation_route_support.py` 로, dispatch planning과 schedule/status response composition은 `web/generation_route_dispatch.py` 로 이동해 endpoint 의미와 HTTP semantics는 유지한 채 hotspot 책임을 줄입니다.
