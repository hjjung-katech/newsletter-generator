...
- preset lifecycle 운영 가시성은 `web/routes_presets.py` 가 raw preset persistence를 직접 노출하지 않고 `web/preset_route_support.py` 를 통해 additive `preset_visibility` / `latest_related_execution` / `source_policy_visibility` payload를 만들고, `web/static/js/app_view_state_helpers.js` 가 selected/default/recent-use/source-policy 연결 상태를 같은 render model로 보여주도록 유지합니다.
- source policy 운영 가시성은 `web/routes_source_policies.py` 가 raw CRUD row를 직접 반환하지 않고 `web/source_policy_route_support.py` 를 통해 additive `source_policy_visibility` / `preset_linkage_visibility` / `latest_related_execution` payload를 만들고, `web/static/js/app_view_state_helpers.js` 가 enabled, disabled, detached, recent-use 상태와 preset/generation 연결 상태를 같은 render model로 보여주도록 유지합니다.
- personalization 운영 가시성은 별도 엔진을 만들지 않고 현재 preset params 와 generation `input_params` 를 `web/personalization_route_support.py` 에서 additive `personalization_visibility` 로 정규화하고, `web/static/js/app_view_state_helpers.js` 와 `web/static/js/app_request_response_helpers.js` 가 default/override/source-policy/archive-context 상태를 같은 render model로 보여주도록 유지합니다.
- settings provenance 운영 가시성은 위 개별 visibility를 다시 조합하는 `web/settings_provenance_support.py` 와 route helper layer가 additive `effective_settings_provenance` payload를 만들고, 같은 helper가 detached / mismatched / empty / default-only 상태에 대한 additive diagnostics reason layer도 함께 계산합니다. 현재 diagnostics payload는 reason code/summary 뿐 아니라 field-level explanation(`field_summary`, `field_explanations`)도 포함해 어떤 축과 어떤 필드가 기대값/현재값에서 어긋났는지까지 additive 하게 설명합니다. lineage summary/detail(`lineage.summary`, `lineage.steps`)는 preset / source policy / personalization / recent execution 이 어떤 순서의 provenance chain으로 해석되는지도 additive 하게 보여줍니다. `web/static/js/app_view_state_helpers.js` 와 `web/static/js/app_request_response_helpers.js` 는 이 payload를 같은 provenance summary와 diagnostics summary로 정규화해 사용자가 현재 effective settings 조합과 그 근거를 같은 방식으로 해석하도록 유지합니다. 추가 drill-down/history/causal detail은 baseline이 아니라 reopen 조건이 성립할 때만 optional backlog로 다룹니다.
...
- 2026-03-15 전략 close-out 기준 위 다섯 hotspot(`newsletter/llm_factory.py`, `newsletter/tools.py`, `newsletter/graph.py`, `web/routes_generation.py`, `web/static/js/app.js`)은 maintenance mode 대상으로 고정합니다. 다음부터는 user-facing bug fix, 새로운 제품 요구, 중복 로직 재유입, shell complexity 재상승, 반복된 운영 해석 요청이 optional backlog 범위를 정당화하는 경우에만 reopen 대상으로 다룹니다.
