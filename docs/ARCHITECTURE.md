...
- settings provenance 운영 가시성은 위 개별 visibility를 다시 조합하는 `web/settings_provenance_support.py` 와 route helper layer가 additive `effective_settings_provenance` payload를 만들고, `web/static/js/app_view_state_helpers.js` 와 `web/static/js/app_request_response_helpers.js` 가 preset / source policy / personalization / recent execution 을 하나의 provenance summary로 정규화해 사용자가 현재 effective settings 조합과 그 근거를 같은 방식으로 해석하도록 유지합니다.
