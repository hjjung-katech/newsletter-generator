...
class TestCompose(unittest.TestCase):
    ...
    def test_compose_newsletter_html_success(self):
        summaries = [
            {'title': 'Test Article 1', 'url': 'http://example.com/1', 'summary_text': 'Summary 1'}, # 변경: 'summary' -> 'summary_text'
            {'title': 'Test Article 2', 'url': 'http://example.com/2', 'summary_text': 'Summary 2'}  # 변경: 'summary' -> 'summary_text'
        ]
        ...
        with patch.dict(os.environ, {'GENERATION_DATE': '2025-05-10'}):
            html_content = compose_newsletter_html(summaries, template_dir, template_name)
        ...
        self.assertIn("2025-05-10", html_content) # Check if generation date is in the output
    ...
    def test_compose_newsletter_html_template_not_found(self):
        ...
        with self.assertRaises(Exception): # Expecting a Jinja2 TemplateNotFound error, but Exception is broader
