...
from newsletter.tools import extract_common_theme_fallback, get_filename_safe_theme
...
def test_compose_detailed_newsletter_success(detailed_style_data, template_dir):
    ...
    with patch.dict(
        ...
        html_content = compose_newsletter_html(
            detailed_style_data, template_dir, "newsletter_template.html"
        )
...
def test_compose_detailed_newsletter_empty_summaries(template_dir):
    ...
    with patch.dict(
        ...
        html_content = compose_newsletter_html(
            [], template_dir, "newsletter_template.html"
        )
    assert "2025-05-10" in html_content
    assert "12:34:56" in html_content
...
def test_compose_detailed_template_not_found(detailed_style_data, template_dir):
    ...
    with pytest.raises(Exception):
        compose_newsletter_html(
            detailed_style_data, template_dir, "non_existent_template.html"
        )
...
def test_compose_compact_template_rendering(compact_style_data, template_dir):
    ...
    html = compose_compact_newsletter_html(
        compact_style_data, template_dir, "newsletter_template_compact.html"
    )
...
def test_compact_definitions_extraction():
    ...
    test_sections = [
        ...
        {
            "title": "로보택시 상용화",
            "definitions": [{"term": "로보택시", "explanation": "자율주행 기술 기반 택시 서비스"}],
        },
    ]
...
def test_compact_definitions_extraction_edge_cases():
    ...
    assert (
        extract_key_definitions_for_compact([{"title": "Test", "definitions": []}])
        == []
    )
