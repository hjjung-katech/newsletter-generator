...
class TestScheduleIntegration:
    ...
    def create_test_schedule(
        self, db_path, schedule_id, next_run_str, is_test=1, enabled=1
    ):
        """테스트 스케줄 생성 헬퍼"""
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        ...
        cursor.execute(
            ...
            (id, params, rrule, next_run, created_at, enabled, is_test, expires_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (
                schedule_id,
    ...
    def test_schedule_creation_to_execution_flow(
        ...
        with patch("web.schedule_runner.datetime") as mock_datetime:
            ...
            assert (
                len(pending_schedules) == 1
            ), f"Expected 1 pending schedule, got {len(pending_schedules)}"
    ...
    def test_multiple_schedules_execution_order(
        ...
        with patch("web.schedule_runner.datetime") as mock_datetime:
            ...
            assert (
                len(pending_schedules) == 3
            ), f"Expected 3 schedules, got {len(pending_schedules)}"
            ...
            assert (
                schedule_ids == expected_order
            ), f"Expected {expected_order}, got {schedule_ids}"
    ...
    def test_expired_schedule_cleanup(self, temp_db, schedule_runner_with_db):
        ...
        with patch("web.schedule_runner.datetime") as mock_datetime:
            ...
            cursor.execute(
                "SELECT id, enabled FROM schedules WHERE id = 'expired_schedule'"
            )
            expired_result = cursor.fetchone()

            cursor.execute(
                "SELECT id, enabled FROM schedules WHERE id = 'valid_schedule'"
            )
    ...
    def test_schedule_next_run_update_after_execution(
        self, temp_db, schedule_runner_with_db, mock_newsletter_generation
    ):
        ...
        with patch("web.schedule_runner.datetime") as mock_datetime:
            ...
            ), "next_run should be updated after execution"

            # 새로운 시간이 미래인지 확인
            new_next_run_dt = datetime.fromisoformat(
                new_next_run.replace("Z", "+00:00")
            )
    ...
    def test_test_vs_regular_schedule_windows(self, temp_db, schedule_runner_with_db):
        ...
        self.create_test_schedule(
            temp_db,
            "regular_15min",
            past_time.strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
            is_test=0,
        )
        self.create_test_schedule(
            temp_db,
        ...
        with patch("web.schedule_runner.datetime") as mock_datetime:
            ...
            assert (
                "regular_15min" in pending_ids
            ), "Regular schedule should be ready (within 30min window)"
            assert (
                "test_15min" not in pending_ids
            ), "Test schedule should be expired (beyond 10min window)"

    def test_concurrent_schedule_execution_safety(
        self, temp_db, mock_newsletter_generation
    ):
        """동시 실행 안전성 테스트"""
        ...
        self.create_test_schedule(
            temp_db, "concurrent_test", ready_time.strftime("%Y-%m-%dT%H:%M:%S.%fZ")
        )
