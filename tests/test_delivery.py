...
def test_save_locally(tmp_path):
    ...
    # HTML 저장 테스트
    result_html = news_deliver.save_locally(
        html_content, filename_base, "html", str(output_dir)
    )
    ...
    # Markdown 저장 테스트
    result_md = news_deliver.save_locally(
        html_content, filename_base, "md", str(output_dir)
    )
...
@pytest.mark.skipif(
    not os.getenv("POSTMARK_SERVER_TOKEN"), reason="POSTMARK_SERVER_TOKEN is not set"
)
def test_real_email_sending():
    ...
    subject = (
        f"Newsletter Generator 실제 통합 테스트 - {os.environ.get('GENERATION_DATE', '')}"
    )
    content = "<h1>실제 이메일 테스트</h1><p>이 이메일은 통합 테스트에 의해 발송되었습니다.</p>"

    result = news_deliver.send_email(
        to_email=test_recipient, subject=subject, html_content=content
    )
