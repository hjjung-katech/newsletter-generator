...
def test_schedule_scenarios():
    ...
    return {
        "ready_regular": {
            ...
            "next_run": (base_time - timedelta(minutes=5)).strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
            "is_test": 0,
            "enabled": 1
        },
        "ready_test": {
            "id": "ready_test_001", 
            "next_run": (base_time - timedelta(minutes=3)).strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
            "is_test": 1,
            "enabled": 1
        },
        "future_regular": {
            "id": "future_regular_001",
            "next_run": (base_time + timedelta(hours=2)).strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
            "is_test": 0,
            "enabled": 1
        },
        "expired_regular": {
            "id": "expired_regular_001",
            "next_run": (base_time - timedelta(hours=1)).strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
            "is_test": 0,
            "enabled": 1
        },
        "expired_test": {
            "id": "expired_test_001",
            "next_run": (base_time - timedelta(minutes=20)).strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
            "is_test": 1,
            "enabled": 1
        },
        "disabled": {
            "id": "disabled_001",
            "next_run": (base_time - timedelta(minutes=2)).strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
            "is_test": 0,
            "enabled": 0
        }
    }
...
def populate_test_schedules(temp_schedule_db, test_schedule_scenarios):
    ...
    for scenario_name, scenario_data in test_schedule_scenarios.items():
        ...
        if scenario_data["is_test"]:
            base_time = datetime(2025, 8, 13, 15, 50, 0, tzinfo=timezone.utc)
            expires_at = (base_time + timedelta(hours=1)).strftime("%Y-%m-%dT%H:%M:%S.%fZ")
...
def schedule_execution_monitor():
    ...
    class ExecutionMonitor:
        ...
        def get_execution_count(self, schedule_id=None):
            if schedule_id:
                return len([e for e in self.executions if e['schedule_id'] == schedule_id])
