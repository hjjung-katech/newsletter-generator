...
class TestWebMail(unittest.TestCase):
    ...
    def test_check_email_configuration_complete(self):
        with patch.dict(os.environ, {"POSTMARK_SERVER_TOKEN": "test-token", "EMAIL_SENDER": "test@example.com"}):
    ...
    def test_check_email_configuration_incomplete_token(self, mock_get_config, mock_validate):
    ...
    @patch("newsletter.config_manager.config_manager.validate_email_config")
    @patch("web.mail._get_email_config")
    def test_check_email_configuration_incomplete_sender(self, mock_get_config, mock_validate):
        # config_manager가 ImportError를 일으키도록 설정
        mock_validate.side_effect = ImportError
