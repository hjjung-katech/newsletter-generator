...
class TestScheduleCompatibility:
    ...
    def test_existing_config_not_affected(self):
        ...
        try:
            ...
            ), "Gemini API key settings should exist"
            assert hasattr(
                settings, "llm_request_timeout"
            ), "LLM timeout settings should exist"
            ...
            assert (
                settings_after is not None
            ), "Settings should remain available after schedule import"
    ...
    def test_database_schema_backward_compatibility(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT id, params, is_test FROM schedules WHERE id = 'legacy_schedule'"
            )
    ...
    def test_existing_newsletter_generation_not_affected(self):
        ...
        with (
            patch("newsletter.main.collect_news") as mock_collect,
            ...
            mock_generate.return_value = {
                "status": "success",
                "content": "<html>Test Newsletter</html>",
            }
            ...
            try:
                ...
                assert (
                    mock_generate is not None
                ), "generate_newsletter should be mockable"
    ...
    def test_environment_variables_isolation(self):
        ...
        try:
            ...
            assert (
                os.environ.get("GEMINI_API_KEY") == "test_key"
            ), "Existing env vars should not be modified"
            assert (
                os.environ.get("NEWSLETTER_TEST_MODE") == "true"
            ), "Test mode should not be affected"
    ...
    def test_existing_test_fixtures_compatibility(
        self, mock_google_ai, mock_serper_api
    ):
        ...
        try:
            ...
            assert (
                "organic" in mock_serper
            ), "Serper mock should have expected structure"

            # conftest.py에서 fixture 함수들이 정의되어 있는지 확인
            import tests.conftest as conftest_module

            assert hasattr(
                conftest_module, "mock_google_ai"
            ), "mock_google_ai fixture should be defined"
            assert hasattr(
                conftest_module, "mock_serper_api"
            ), "mock_serper_api fixture should be defined"
        ...
        for module_name in existing_test_modules:
            try:
                ...
                if "schedule" in str(e).lower():
                    pytest.fail(
                        f"Scheduling changes broke existing test: {module_name} - {e}"
                    )
                # 다른 이유 (API 키 없음 등)는 경고만
                pytest.skip(
                    f"Existing test import issue (not related to scheduling): {module_name} - {e}"
                )
