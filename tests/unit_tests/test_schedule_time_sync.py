...
class TestScheduleTimeSync:
    ...
    def test_time_diff_calculation(self):
        ...
        test_cases = [
            {
                ...
                "next_run": (fixed_now - timedelta(minutes=5)).strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ"
                ),
                "is_test": False,
            ...
            },
            {
                ...
                "next_run": (fixed_now - timedelta(minutes=3)).strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ"
                ),
                "is_test": True,
                "expected_status": "READY",
            },
            {
                ...
                "next_run": (fixed_now - timedelta(minutes=35)).strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ"
                ),
                "is_test": False,
            ...
            },
            {
                ...
                "next_run": (fixed_now - timedelta(minutes=15)).strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ"
                ),
                "is_test": True,
                "expected_status": "EXPIRED",
            },
            {
                ...
                "next_run": (fixed_now + timedelta(hours=1)).strftime(
                    "%Y-%m-%dT%H:%M:%S.%fZ"
                ),
                "is_test": False,
        ...
        for test_case in test_cases:
            next_run_dt = datetime.fromisoformat(
                test_case["next_run"].replace("Z", "+00:00")
            )
            time_diff = fixed_now - next_run_dt

            # 실행 창 결정
            execution_window = (
                timedelta(minutes=10) if test_case["is_test"] else timedelta(minutes=30)
            )
            ...
            ), f"{description}: expected {expected_status}, got {actual_status}"
    ...
    def test_execution_window_logic(self):
        ...
        for is_test, minutes_ago, should_execute in test_cases:
            ...
            # 실행 창 계산
            execution_window = (
                timedelta(minutes=10) if is_test else timedelta(minutes=30)
            )
            ...
            assert (
                is_ready == should_execute
            ), f"Test={is_test}, {minutes_ago}min ago: expected {should_execute}, got {is_ready}"
    ...
    def test_timezone_parsing(self):
        ...
        for iso_string, should_be_aware in test_cases:
            ...
            ) == should_be_aware, f"Timezone awareness mismatch for {iso_string}"
    ...
    def test_schedule_classification(self, temp_db):
        ...
        for schedule in schedules:
            ...
            # 실행 창 결정
            execution_window = (
                timedelta(minutes=10) if schedule["is_test"] else timedelta(minutes=30)
            )
            ...
            ), f"Schedule {schedule['id']}: expected {expected}, got {actual_status}"
    ...
    def test_next_run_calculation(self):
        ...
        test_cases = [
            ...
            (
                ...
                datetime(2025, 8, 13, 14, 0, tzinfo=timezone.utc),
                15,
            ),  # 오늘 15:30
        ]
        ...
        for rrule_str, current_time, expected_hour in test_cases:
            ...
            assert (
                next_run.hour == expected_hour
            ), f"Expected hour {expected_hour}, got {next_run.hour}"
            assert (
                next_run > current_time
            ), f"Next run should be in future: {next_run} <= {current_time}"
    ...
    def test_test_schedule_expiry(self, temp_db):
        ...
        expired_time = datetime.fromisoformat(
            "2025-08-13T15:30:00Z".replace("Z", "+00:00")
        )
        valid_time = datetime.fromisoformat(
            "2025-08-13T16:30:00Z".replace("Z", "+00:00")
        )
...
class TestTimeUtilities:
    ...
    def test_timezone_consistency(self):
        ...
        # 9시간 차이여야 함 (오차 범위 1분)
        assert abs(time_diff - expected_diff) < timedelta(
            minutes=1
        ), f"KST should be UTC+9, got diff: {time_diff}"
...
class TestScheduleExecutionLogic:
    ...
    def test_execution_window_boundaries(self):
        ...
        for minutes_ago, is_test, expected_ready in boundary_cases:
            ...
            ), f"Boundary test failed: {minutes_ago}min ago, test={is_test}"
