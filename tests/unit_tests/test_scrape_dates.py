...
source_manager.add_source(SerperAPISource())

# 뉴스 기사 수집
print(f"키워드 '{', '.join(test_keywords)}'로 뉴스 기사 수집 중...")
articles = source_manager.fetch_all_sources(test_keywords, num_results_per_source=3)
...
for idx, article in enumerate(articles[:10]):  # 처음 10개만 표시
    ...
    # 날짜 정보 출력
    formatted_date = format_date_for_display(date_str=date) if date else "날짜 없음"
